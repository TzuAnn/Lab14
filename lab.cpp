...
void printAllRecords(fstream &File)
{
    ...
    while ( !File.eof() )
    {
        ...
        if ( client.getAccountNumber() != 0) // skip empty records
            cout << left << setw(10) << client.getAccountNumber() << setw(16)
        << client.getLastName() << setw(11) << client.getFirstName() << right
        << setw( 10 ) << client.getBalance() << endl;
...
int newRecord( fstream &insertInFile ) //  automatically find an account number that is not used for creating a new record. 
{
    ...
    if ( client.getAccountNumber() == 0 )
    {
        ...
        cin >> balance;
        cout << "Successfully:  " << accountNumber << endl;
